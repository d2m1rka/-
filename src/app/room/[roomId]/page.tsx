...
const { mutate: destroyRoom } = useMutation({
    ...
    <header className="border-b border-zinc-800 p-4 flex items-center justify-between bg-zinc-900/30">
            <div className="flex items-center gap-4">
                <div className="flex flex-col">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={copyLink} className="text-[10px] bg-zinc-800 hover:bg-zinc-700 px-2 py-0.5 rounded
                         text-zinc-400 hover:text-zinc-200 transition-colors">{copyStatus}</button>
                    </div>
                </div>
                ...
                <div className="flex flex-col">
                    <span className="text-xs text-zinc-500 uppercase">Само-Уничтожение</span>
            ...
            <button onClick={() => destroyRoom()} className="text-xs bg-zinc-800 hover:bg-red-600 px-3 py-1.5 rounded 
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
            {messages?.messages.length === 0 && (
               <div className="flex items-center justify-center h-full">
                    <p className="text-zinc-600 text-sm font-mono">Пока нет сообщений, начните беседу.</p>
               </div> 
            )}
                        ...
                        <div className="flex items-baseline gap-3 mb-1">
                            <span className={`text-xs font-bold ${
                                msg.sender === username ? "text-green-500" : "text-blue-500"}`}>
                                    {msg.sender === username ? "ВЫ" : msg.sender}
                                </span>
        ...
        <div className="p-4 border-t border-zinc-800 bg-zinc-900/30">
            <div className="flex gap-4">
                ...
                className="bg-zinc-800 text-zinc-400 px-6 text-sm font-bold hover:text-zinc-200 transition-all
                 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:text-zinc-400 cursor-pointer">ОТПРАВИТЬ</button>
            </div>
        </div>
    </main>
}
